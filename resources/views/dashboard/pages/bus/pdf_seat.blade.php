        ...
        <div class="container-fluid">
            ...
            <div class="card col-md-4 mx-auto mt-5">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-7">
                            <h1>Bus {{ $row->id_bus.' - '.$row->trayek->jurusan }}</h1>
                            <h6>{{ $row->trayek->rute }}</h6>
                        </div>
                        <div class="col-3 mt-2 ml-4">
                            Total kursi : {{ $row->total_kursi }} kursi <br>
                            Total tersedia : {{ $row->total_kursi - $peserta->where('bus_id', $row->id_bus)->count() }} kursi <br>
                            Total dipesan : {{ $peserta->where('bus_id', $row->id_bus)->count() }} kursi
                        </div>
                    </div>
                                ...
                                <center>
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 col-6 my-2">
                                            <label class="bg-warning text-white rounded border border-warning p-2 w-100" for="seat{{ $i . $kode . $row }}" style="height: 10vh;">
                                        ...
                                        <div class="col-md-6 col-6 my-2">
                                            <label class="bg-danger text-white rounded border border-danger p-2 w-100" for="seat{{ $i . $kode . $row }}" style="height: 10vh">
                                        ...
                                        @else
                                        <div class="col-md-6 col-6 my-2">
                                            <label class="bg-success text-white rounded border border-success p-2 w-100" for="seat{{ $i . $kode . $row }}" style="height: 10vh;">
                            ...
                            @for ($i = 1; $i <= $row->seat_kiri; $i++)
                                <center>
                                    <div class="row">
                                        ...
                                        @if ($seatCek->where('seat_booked', $seatCode)->where('status', 'book')->isNotEmpty())
                                        <div class="col-md-6 col-6 my-2">
                                            <label class="bg-warning text-warning rounded border border-warning p-2 w-100" for="seat{{ $i . $kode . $row }}" style="height: 10vh;">
                                        ...
                                        @elseif ($seatCek->where('seat_booked', $seatCode)->where('status', 'full')->isNotEmpty())
                                        <div class="col-md-6 col-6 my-2">
                                            <label class="bg-danger text-danger rounded border border-danger p-2 w-100" for="seat{{ $i . $kode . $row }}" style="height: 10vh;">
                                                <b class="text-danger">{{ $i . $kode }}</b>
                                                <small class="text-dark">
                                                    <h6 class="mb-0">{{ $peserta->where('bus_id', $bus)->where('kode_seat', $i.$kode)->first()?->nama_peserta }}</h6>
                                                    <h6 class="mt-0">{{ $peserta->where('bus_id', $bus)->where('kode_seat', $i.$kode)->first()?->booking->uker->nama_unit_kerja }}</h6>
                                        ...
                                        @else
                                        <div class="col-md-6 col-6 my-2">
                                            <label class="bg-success text-success rounded border border-success p-2 w-100" for="seat{{ $i . $kode . $row }}" style="height: 10vh;">
                                                <div class="pt-2"><b class="text-success">{{ $i . $kode }}</b></div>
                                            </label>
                                        </div>
                                        @endif
